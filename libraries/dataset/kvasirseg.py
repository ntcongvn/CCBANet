...
def LoadKvasirSegDataset(config):
  data=[]
  with open(config['dataset_dir']+'/Kvasir-SEG/kavsir_bboxes.json') as json_file:
  ...
  def __init__(self,config,data,mode="train",normalization=True,augmentation=False):
    ...
    for index in np.arange(len(self.data)):
      ...
      if self.mode=="train" or self.mode=="val" or self.mode=="test":
        self.image_paths.append(config["dataset_dir"]+"/Kvasir-SEG/images/"+d["image"])
        self.label_paths.append(config["dataset_dir"]+"/Kvasir-SEG/masks/"+d["image"])        
  ...
  def __getitem__(self, index):
    if self.mode=="train":   
      ...
      if self.augmentation is True:
        ...
        image_data=A.Resize(height=320, width=320, interpolation=cv2.INTER_LINEAR,p=1)(image=image_data)["image"]
        mask_data=A.Resize(height=320, width=320, interpolation=cv2.INTER_NEAREST,p=1)(image=mask_data)["image"]
